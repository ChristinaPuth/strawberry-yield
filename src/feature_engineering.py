...
#     "days_since_last",   # v4.1: moved from metadata into features
# ]
# SPATIAL_FEATS = [
...
#     meta_cols     = []   # days_since_last is now in TEMPORAL_FEATS (ALL_FEATS)
...
# #         gaps[harvest_dates[i]] = (harvest_dates[i] - harvest_dates[i-1]).days
# #     df["days_since_last"] = df["harvest_date"].map(gaps)
# #     # Fill NaN for first harvest (no previous date) with median interval
# #     median_interval = df["days_since_last"].median()
# #     df["days_since_last"] = df["days_since_last"].fillna(median_interval)
...
def coarsen_grid(df: pd.DataFrame, n: int = 3,
                 nx: int = None, ny: int = None) -> pd.DataFrame:
    """
    Aggregate grid cells into super-cells.

    Parameters
    ----------
    n   : shorthand for square super-cells (nx=ny=n). Ignored if nx/ny given.
    nx  : coarsening factor along field_x axis
    ny  : coarsening factor along field_y axis

    For row-oriented fields (垄沿 field_y 方向):
        along-row    : nx small, ny large  e.g. nx=2, ny=8
        across-row   : nx large, ny small  e.g. nx=8, ny=2
    """
    ...
    label = f"{nx}x{ny}"
    print(f"\nCoarsening grid: {label} super-cells ...")
    original_rows = len(df)
...
def build_features(df: pd.DataFrame,
    ...
    id_cols       = ["site","harvest_date","harvest_idx",
                     "field_x","field_y","easting","northing"]
    meta_cols     = ["days_since_last"]   # metadata only — NOT in ALL_FEATS, NOT a model input
    baseline_cols = ["seasonal_mean","trend_extrap"]
    keep = [c for c in id_cols + meta_cols + ["weight_kg"]
            + ALL_FEATS + baseline_cols if c in df.columns]
    df = df[keep].reset_index(drop=True)
