...
def _get_grid(df_day):
    ...
    for _, row in df_day.iterrows():
        grid[yi[row['field_y']], xi[row['field_x']]] = row['weight_kg']
    return grid
...
def plot_yield_map_grid(df, site, harvest_date, ax=None, title=None):
    ...
    ax.set_title(title or f'{site}  —  {str(harvest_date)[:10]}', fontsize=11, fontweight='bold')
...
def plot_yield_map_utm(df, site, harvest_date, ax=None, title=None, s=3):
    ...
    ax.set_title(title or f'{site}  —  {str(harvest_date)[:10]}  (UTM)', fontsize=11, fontweight='bold')
...
def plot_distribution(df, site, harvest_date=None):
    ...
    fig.suptitle(f'{site}  —  Yield distribution  ({sub})', fontsize=13, fontweight='bold')
    ...
    axes[1].axvline(nonzero['weight_kg'].median(), color='navy', linestyle='--',
                    linewidth=1.5, label=f'Median: {nonzero["weight_kg"].median():.3f} kg')
    axes[1].axvline(nonzero['weight_kg'].mean(), color='green', linestyle='--',
                    linewidth=1.5, label=f'Mean: {nonzero["weight_kg"].mean():.3f} kg')
